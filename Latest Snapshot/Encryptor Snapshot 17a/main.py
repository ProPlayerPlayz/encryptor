...
sizel = {"1":[12,24,18],"2":[14,26,20],"3":[16,26,22]}
fontl = ["Arial","Courier New","Comic Sans MS","Consolas","Times New Roman"]
...
def submit_options():
    ...
    if presize in sizel:
        Tsize = sizel[presize][0]
        tsize = sizel[presize][1]
        msize = sizel[presize][2]
...
# Options Window
def Options_func():
    optionwin = tk.Toplevel()
    optionwin.title("Options")
    optionwin.iconbitmap(r"iconlocked.ico")
    optionwin.resizable(0,0)
    tk.Label(optionwin,text = "Options",font = ('Consolas',24)).grid(row = 0,column = 0)
    
    global sizevar
    global fontvar
    fontvar = tk.StringVar()
    fontvar.set("Consolas")
    sizevar = tk.StringVar()
    sizevar.set("1")
    Frame1 = tk.Frame(optionwin)
    tk.Label(Frame1,text = "Font:").grid(row = 0,column = 0)
    tk.OptionMenu(Frame1,fontvar, *fontl).grid(row = 0,column = 1)
    tk.Label(Frame1,text = "Size:").grid(row = 1,column = 0)
    tk.OptionMenu(Frame1,sizevar, *sizel).grid(row = 1,column = 1,ipadx = 20.5)
    Frame1.grid(row = 1)
    
    tk.Button(optionwin,text = "Save",command = submit_options).grid(row = 2)
    tk.Label(optionwin, text = "Restart is required for changes to appear")
...
tk.Button(root,text = "Options",command = Options_func,font = ('Consolas',18,'bold'), padx = 7).grid(row = 3,column = 0)
root.mainloop()
